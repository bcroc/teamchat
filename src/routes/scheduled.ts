...
export const scheduledRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{ Body: z.infer<typeof createScheduledMessageSchema> }>(
    '/',
    async (request, reply) => {
      const result = createScheduledMessageSchema.safeParse(request.body);
      if (!result.success) {
        throw errors.validation('Invalid input', { errors: result.error.flatten() });
      }

      const { workspaceId, channelId, dmThreadId, body, scheduledAt } = result.data;
  ...
  fastify.patch<{ Params: { id: string }; Body: z.infer<typeof updateScheduledMessageSchema> }>(
    ...
    async (request, reply) => {
      const { id } = request.params;
      const result = updateScheduledMessageSchema.safeParse(request.body);
      ...
      if (result.data.body) {
        updateData.body = result.data.body;
      }
