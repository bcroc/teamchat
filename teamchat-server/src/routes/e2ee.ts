...
export const e2eeRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{ Params: { userId: string } }>(
    ...
    async (request) => {
      ...
      const requesterWorkspaces = await prisma.workspaceMember.findMany({
        where: { userId: request.user.id },
        select: { workspaceId: true },
      });

      const workspaceIds = requesterWorkspaces.map((w) => w.workspaceId);
      if (workspaceIds.length === 0) {
        throw errors.forbidden('Not authorized to access encryption keys');
      }

      const sharedWorkspace = await prisma.workspaceMember.findFirst({
        where: {
          userId,
  ...
  fastify.get<{ Params: { conversationId: string } }>(
    ...
    async (request) => {
      ...
      if (channel) {
        await requireChannelAccess(request.user.id, conversationId);
      } else {
        ...
        await requireDmAccess(request.user.id, conversationId);
      }
