...
export const reminderRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{ Body: z.infer<typeof createReminderSchema> }>(
    '/',
    async (request, reply) => {
      const { workspaceId, messageId, text, remindAt } = assertZodSuccess(
        createReminderSchema.safeParse(request.body)
      );
  ...
  fastify.patch<{ Params: { id: string }; Body: z.infer<typeof updateReminderSchema> }>(
    ...
    async (request, reply) => {
      const { id } = request.params;
      const result = assertZodSuccess(updateReminderSchema.safeParse(request.body));
      ...
      if (result.text) {
        updateData.text = result.text;
      }

      if (result.remindAt) {
        const newDate = new Date(result.remindAt);
        if (newDate <= new Date()) {
          throw errors.validation('Reminder time must be in the future');
        }
