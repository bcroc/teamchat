...
export const savedRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', async (request) => {
    const { cursor, limit } = assertZodSuccess(
      getSavedMessagesSchema.safeParse(request.query)
    );
  ...
  fastify.post('/', async (request, reply) => {
    const { messageId, note } = assertZodSuccess(
      saveMessageSchema.safeParse(request.body)
    );
