...
export const preferencesRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.patch<{ Body: z.infer<typeof updatePreferencesSchema> }>(
    '/',
    async (request, reply) => {
      const result = assertZodSuccess(
        updatePreferencesSchema.safeParse(request.body)
      );
  ...
  fastify.patch<{ Params: { channelId: string }; Body: z.infer<typeof updateChannelSettingsSchema> }>(
    ...
    async (request, reply) => {
      const { channelId } = request.params;
      const result = assertZodSuccess(
        updateChannelSettingsSchema.safeParse(request.body)
      );
      ...
      // Handle muteUntil
      if (result.muteUntil !== undefined) {
        updateData.muteUntil = result.muteUntil ? new Date(result.muteUntil) : null;
      }
